...
package body Liblktlang.Unparsing_Implementation is
   ...
   function Extract_Regular_Node_Template
     (Unparser       : Regular_Node_Unparser;
      Rewritten_Node : Bare_Lkt_Node) return Regular_Node_Template;
   ...
   function Create_Abstract_Node
     (Parsing_Node : Bare_Lkt_Node) return Abstract_Node is
   begin
   ...
   function Kind (Node : Abstract_Node) return Lkt_Node_Kind_Type is
   begin
      case Node.Kind is
         when From_Parsing =>
   ...
   function Rewritten_Node
     (Node : Abstract_Node) return Bare_Lkt_Node is
   begin
      case Node.Kind is
         when From_Parsing =>
   ...
   begin
      ...
      for I in 1 .. Children_Count (Rewritten_Node) loop
         declare
            ...
            Rewritten_Child : constant Bare_Lkt_Node :=
   ...
   begin
      while Has_Element (Cursor) loop
         ...
         if I > 1 and then Unparser.Separator /= null then
            ...
            then
               declare
                  BN_Child : constant Bare_Lkt_Node :=
                     Child (Rewritten_Node, I);
