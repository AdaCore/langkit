...
testcases = (
    ...
    Testcase(
        "comment_box_middle_line_shorter.py",
        """
        #######
        # Box#
        #######
    ...
    Testcase(
        "comment_box_last_line_shorter.py",
        """
        #######
        # Box #
        ######
    ...
    """,
        [
            (
                2,
                0,
                "Two empty lines required between the last import statement"
                " and this line.",
            )
        ],
    ),
...
def reindent_content(tc: Testcase) -> str:
    ...
    assert not lines[0], "First content line for {} must be empty".format(
        tc.filename
    )
    result = []
    indent = " " * 8
    for i, line in enumerate(lines[1:], 1):
        assert (
            not line.strip() or line[: len(indent)] == indent
