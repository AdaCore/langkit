...
package body Langkit_Support.Adalog.Solver is
   ...
   procedure Default_Converter_Inc_Ref (Dummy : Default_Converter) is null;
   procedure Default_Converter_Dec_Ref
     (Dummy : in out Default_Converter) is null;
   ...
   procedure Default_Eq_Data_Inc_Ref (Dummy : Default_Eq) is null;
   procedure Default_Eq_Data_Dec_Ref (Dummy : in out Default_Eq) is null;

   package SSM_Unify is new SSM_Solve.Raw_Custom_Bind
     (Converter           => Default_Converter,
      No_Data             => (null record),
      Converter_Inc_Ref   => Default_Converter_Inc_Ref,
      Converter_Dec_Ref   => Default_Converter_Dec_Ref,
      Equals_Data         => Default_Eq,
      No_Equals_Data      => (null record),
      Equals_Data_Inc_Ref => Default_Eq_Data_Inc_Ref,
      Equals_Data_Dec_Ref => Default_Eq_Data_Dec_Ref,
      Convert             => Convert,
      Equals              => Equals,
   ...
   procedure Comparer_Inc_Ref (Self : Comparer_Access);
   procedure Comparer_Dec_Ref (Self : in out Comparer_Access);
   ...
   procedure Converter_Inc_Ref (Self : Converter_Access);
   procedure Converter_Dec_Ref (Self : in out Converter_Access);
   ...
   package SSM_Bind_One_Side is new SSM_Solve.Raw_Custom_Bind
     (Converter           => Converter_Access,
      Converter_Inc_Ref   => Converter_Inc_Ref,
      Converter_Dec_Ref   => Converter_Dec_Ref,
      No_Data             => null,
      Equals_Data         => Comparer_Access,
      No_Equals_Data      => null,
      Equals_Data_Inc_Ref => Comparer_Inc_Ref,
      Equals_Data_Dec_Ref => Comparer_Dec_Ref,
      Convert             => Convert,
      Equals              => Equals,
   ...
   procedure Comparer_Inc_Ref (Self : Comparer_Access) is
   begin
      if Self /= null then
         Self.Ref_Count := Self.Ref_Count + 1;
      end if;
   end Comparer_Inc_Ref;

   ----------------------
   -- Comparer_Dec_Ref --
   ----------------------

   procedure Comparer_Dec_Ref (Self : in out Comparer_Access) is
      procedure Free is new Ada.Unchecked_Deallocation
        (Solver_Ifc.Comparer_Type'Class, Comparer_Access);
   begin
      if Self = null then
         return;
   ...
   end Comparer_Dec_Ref;

   -----------------------
   -- Converter_Inc_Ref --
   -----------------------

   procedure Converter_Inc_Ref (Self : Converter_Access) is
   begin
      if Self /= null then
         Self.Ref_Count := Self.Ref_Count + 1;
      end if;
   end Converter_Inc_Ref;

   -----------------------
   -- Converter_Dec_Ref --
   -----------------------

   procedure Converter_Dec_Ref (Self : in out Converter_Access) is
      procedure Free is new Ada.Unchecked_Deallocation
        (Solver_Ifc.Converter_Type'Class, Converter_Access);
   begin
      if Self = null then
         return;
      elsif Self.Ref_Count = 1 then
   ...
   begin
      if Debug then
         if Self.Vars = null then
            Self.Vars := new Ref_Counted_Var_Set'(Vars => <>, Ref_Count => 1);
         end if;
         Self.Vars.Vars.Include (Image (V), V);
      end if;
   end Append_Var;
   ...
   begin
      case Self.Kind is
         when Symbolic =>
            Sym_Solve.Inc_Ref (Self.Symbolic_Relation);

         when State_Machine =>
            Abstract_Relation.Inc_Ref (Self.SSM_Relation);
            if Self.Vars /= null then
               Self.Vars.Ref_Count := Self.Vars.Ref_Count + 1;
            end if;
   ...
   procedure Dec_Ref (Self : in out Relation) is
      procedure Free is new Ada.Unchecked_Deallocation
        (Ref_Counted_Var_Set, Var_Set);
   begin
      case Self.Kind is
         when Symbolic =>
            Sym_Solve.Dec_Ref (Self.Symbolic_Relation);

         when State_Machine =>
            Abstract_Relation.Dec_Ref (Self.SSM_Relation);
            if Self.Vars /= null then
               Self.Vars.Ref_Count := Self.Vars.Ref_Count - 1;
               if Self.Vars.Ref_Count = 0 then
                  Free (Self.Vars);
               end if;
            end if;
            ...
            begin
               ...
               return Rel : Relation :=
                 (State_Machine,
                  ...
                  Vars => <>)
               do
                  --  The above call to Create is supposed to create ownership
                  --  shares for C and E, so get rid of ours.
         ...
         when State_Machine =>
            ...
            begin
               ...
               do
                  ...
                  --  The above calls to Create are supposed to create
                  --  ownership shares for C and E, so get rid of ours.
               ...
               return Rel : Relation (State_Machine) do
                  ...
                  for J in Relations'Range loop
                     ...
                     if Relations (J).Vars /= null then
                        for El in Relations (J).Vars.Vars.Iterate loop
