...
package body Langkit_Support.Adalog.Symbolic_Solver is
   ...
   function Simplify
     (Self     : Relation;
      ...
      Sort_Ctx : in out Sort_Context) return Relation
   is
      Outer_Count, Inner_Count : Natural := 0;
      --  Number of times we went through respectively the outer loop (in
      --  ``Simplify``) and the inner loop (in ``Simplify.Process``), for
      --  logging purposes.

      Continue : Boolean := True;
      --  We run the simplification algorithm as many times as possible until
      --  the relation tree does not change anymore. This boolean says whether
      --  we should continue to run it one more time.
      ...
      function Process (Self : Relation) return Relation is
         ...
         --  We run a fixed-point algorithm: as long as atoms (including
         --  Unify ones) keeps growing, look for new contradictions in
         --  ``Anys``. Everytime the simplification of an ``Any`` relation adds
         --  items to ``Atoms`` or ``Unifies``, new opportunities to find
         --  contradictions in ``Anys` may aride. This boolean keeps track of
         --  whether ``Atoms`` or ``Unifies`` growed since the last iteration.
      ...
      begin
         ...
         Fixedpoint_Loop : while Atoms_Changed loop
            ...
            if Simplify_Trace.Is_Active then
               Simplify_Trace.Trace ("Inner iteration" & Inner_Count'Image);
            end if;
   ...
   begin
      ...
      --  As long as ``Process`` manages to simplify the relation, keep running
      --  it, as simplification may create more opportunities to simplify
      --  further.
      ...
      while Continue loop
         ...
         if Simplify_Trace.Is_Active then
            Simplify_Trace.Trace ("Outer iteration" & Outer_Count'Image);
         end if;
         declare
      ...
      if Simplify_Trace.Is_Active then
         ...
         Simplify_Trace.Trace ("Outer iterations:" & Outer_Count'Image);
         Simplify_Trace.Trace ("Inner iterations:" & Inner_Count'Image);
      end if;
   ...
   is
      ...
      begin
         ...
         if not Ctx.Anys.Is_Empty then
            ...
            return Result : constant Boolean := Try_Solution (Ctx.Atoms) do
               Cleanup_Aliases;
            end return;
         end if;
      end Recurse;
