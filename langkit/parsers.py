...
class Layout(Parser):
    """
    Parser class used to track layout events, such as new lines, indents, or
    dedents. For this parser to be used, the `track_indent` option needs to be
    passed to the lexer instantiation.
    """
    ...
    def generate_code(self, pos_name="pos"):
        with self.error_context():
            check_source_language(
                get_context().lexer.track_indent,
                "Using a layout parser with mode {}, but indent "
                "tracking is not activated in the lexer".format(
        ...
        return ParserCodeContext(
            pos, None,
            render('parsers/layout_code_ada',
                   pos=pos, pos_name=pos_name, mode=self.mode),
            [(pos, Token)]
        )
...
def nl():
    """
    Helper to parse newlines if indentation tracking is activated.
    """
    return Layout(mode=Layout.NEWLINE)


def ind():
    """
    Helper to parse indent events if indentation tracking is activated.
    """
    return Layout(mode=Layout.INDENT)


def ded():
    """
    Helper to parse dedent events if indentation tracking is activated.
    """
    return Layout(mode=Layout.DEDENT)


def nod():
    """
    Helper to parse nodent events if indentation tracking is activated.
    """
    return Layout(mode=Layout.NODENT)
