...
with Liblktlang_Support.Adalog.Solver.Diagnostics;
with Liblktlang_Support.Images;

with Liblktlang_AdaSAT.Builders;
...
package body Liblktlang_Support.Adalog.Solver is
   ----------------------
   -- Supporting types --
   ----------------------

   package Atomic_Relation_Vectors is new Liblktlang_Support.Vectors
     (Atomic_Relation);
   ...
   package Internal_Diagnostics is
     new Liblktlang_Support.Adalog.Solver.Diagnostics;
   ...
   package Positive_Vectors is new Liblktlang_Support.Vectors (Positive);
   ...
   type Atom_Mapping is array (Positive range <>)
     of Liblktlang_AdaSAT.Variable_Or_Null;
   --  Array type used to map basic block ids to Liblktlang_AdaSAT formula variables
   ...
   package BB_Vectors is new Liblktlang_Support.Vectors
   ...
   package Unified_Vars_Vectors is new Liblktlang_Support.Vectors
     (Unified_Vars);
   ...
   function Evaluate_Atoms
     (Ctx          : in out Solving_Context;
      ...
      Explanation  : in out Liblktlang_AdaSAT.Builders.Formula_Builder) return Boolean;
   ...
   function Explain_Contradiction
     (Ctx          : Solving_Context;
      ...
      Error_Ctxs   : Logic_Context_Vector_Access := null) return Liblktlang_AdaSAT.Clause;
   ...
   procedure Explain_Topo_Sort_Failure
     (Ctx          : in out Solving_Context;
      Model        : Liblktlang_AdaSAT.Model;
      Explanation  : in out Liblktlang_AdaSAT.Builders.Formula_Builder);
   ...
   function Check
     (Ctx            : in out Solving_Context;
      Model          : Liblktlang_AdaSAT.Model;
      Contradictions : in out Liblktlang_AdaSAT.Formulas.Formula) return Boolean;
   ...
   function Encode_Relation
     (Self           : Compound_Relation;
      Ctx            : in out Solving_Context;
      Variable_Count : out Liblktlang_AdaSAT.Variable_Or_Null)
      return Liblktlang_AdaSAT.Formulas.Formula;
   ...
   package Adalog_Theory is new Liblktlang_AdaSAT.Theory (Solving_Context, Check);
   package DPLL_Adalog is new Liblktlang_AdaSAT.DPLL (Adalog_Theory);
   ...
   function Explain_Contradiction
     (Ctx          : Solving_Context;
      ...
      Error_Ctxs   : Logic_Context_Vector_Access := null) return Liblktlang_AdaSAT.Clause
   is
      use Liblktlang_AdaSAT;

      Reason   : Liblktlang_AdaSAT.Builders.Clause_Builder;
   ...
   is
      ...
      Last_Clause  : Liblktlang_AdaSAT.Clause;
   begin
      for Atom of Sorted_Atoms loop
         if Solv_Trace.Is_Active then
            Solv_Trace.Trace (Image (Atom));
         ...
         if not Solve_Atomic (Atom) then
            ...
            if Success then
               ...
               Liblktlang_AdaSAT.Free (Last_Clause);
            end if;
   ...
   is
      ...
      procedure Contradict_Unset_Var (V : Logic_Var) is
         V_Id   : constant Natural := Id (V);
         Result : Liblktlang_AdaSAT.Builders.Clause_Builder;
      begin
         if Verbose_Trace.Is_Active then
            Verbose_Trace.Trace
   ...
   is
      ...
      Problem  : Liblktlang_AdaSAT.Builders.Formula_Builder;
      ...
      begin
         if Rels.Length = 0 then
            ...
            declare
               ...
               CB : Liblktlang_AdaSAT.Builders.Clause_Builder;
   ...
   function Image_Header (Self : Relation) return String is
      Prefix : constant String :=
        (if Self.Id = 0
         then ""
         else "[" & Liblktlang_Support.Images.Stripped_Image (Self.Id) & "] ");
...
end Liblktlang_Support.Adalog.Solver;
