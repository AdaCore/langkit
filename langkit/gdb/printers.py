...
class ASTNodePrinter(BasePrinter):
    ...
    def node_to_string(self) -> str:
        loc = (
            "synthetic from {}".format(self.parent)
            if self.synthetic
            else self.sloc()
        )
...
class RecordAccessMatcher:
    ...
    def __init__(
        self, record_type_name: str, access_type_name: str | None
    ) -> None:
...
class LexicalEnvPrinter(BasePrinter):
    ...
    def children(self) -> Iterator[tuple[str, str | gdb.Value]]:
        ...
        if env.kind == "orphaned":
            ...
            array = array_ptr.cast(
                element_type.array(lower_bound, upper_bound)
            )
...
class EnvGetterPrinter(BasePrinter):
    ...
    def matches(cls, value: gdb.Value, context: Context) -> bool:
        return value.type.code == gdb.TYPE_CODE_STRUCT and (
            value.type.name
            == "{}.analysis.ast_envs.env_getter".format(context.lib_name)
        )
    ...
    def image(self) -> str:
        ...
        if self.is_dynamic:
            ...
            return (
                "{}.{}".format(node, resolver_name)
                if resolver_name
                else "{} ({})".format(resolver, node)
            )
        else:
            return str(self.env)
...
class ReferencedEnvPrinter(BasePrinter):
    ...
    def matches(cls, value: gdb.Value, context: Context) -> bool:
        return value.type.code == gdb.TYPE_CODE_STRUCT and (
            value.type.name
            == "{}.analysis.ast_envs.referenced_env".format(context.lib_name)
        )
...
class RebindingsPrinter(BasePrinter):
    ...
    def inner(self) -> str:
        ...
        return "[{}]".format(
            ", ".join(rebinding_img(r) for r in reversed(rebindings))
        )
...
class ArrayPrettyPrinter(BasePrinter):
    ...
    @classmethod
    def element_typename(
        cls, struct_typename: str, context: Context
    ) -> str | None:
        """
        ...
        if struct_typename.startswith(prefix) and struct_typename.endswith(
            suffix
        ):
    ...
    def value_to_element_typename(
        cls, value: gdb.Value, context: Context
    ) -> str | None:
        """
    ...
    def to_string(self) -> str:
        ...
        return "{} array of length {}".format(
            adaify_name(self.context, elt_typename), self.length
        )
...
class LangkitVectorPrinter(BasePrinter):
    ...
    def matches(cls, value: gdb.Value, context: Context) -> bool:
        return (
            ...
            and (
                set(f.name for f in value.type.fields())
    ...
    def element_type(self) -> gdb.Type:
        ...
        assert (
            array_access_type.code == gdb.TYPE_CODE_TYPEDEF
    ...
    def to_string(self) -> str:
        return "{} vector of length {}".format(
            adaify_name(self.context, self.element_type.name), self.length
        )
