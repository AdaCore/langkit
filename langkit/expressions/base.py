        ...
        def expand_expr(
            ...
            if isinstance(expr, NullCond.Check):
                return NullCond.record_check(
                    expr.location, checks, expand_expr(expr._expr, checks)
                )

            def recurse(v: object) -> object:
                if isinstance(v, NullCond.Prefix):
                    if isinstance(v, NullCond.Check):
                        v = NullCond.record_check(
                            v.location, checks, expand_expr(v._expr, checks)
                        )
                    assert isinstance(v, NullCond.Prefix)
                    return expand_expr(v._expr, checks)

                else:
                    return expand(v)

            # Prefix detection works only for abstract expressions that are
            # direct attributes of ``v``.
            if expr.is_wrapper_abstract_expression:  # type: ignore
                expr.args = tuple(  # type: ignore
                    recurse(v) for v in expr.args  # type: ignore
                )
                expr.kwargs = {  # type: ignore
                    k: recurse(v)
                    for k, v in expr.kwargs.items()  # type: ignore
                }
            else:
                for k, v in expr.__dict__.items():
                    expr.__dict__[k] = recurse(v)
...
class NullCond:
    ...
    * [Back to depth 1] That was a checked prefix: a new variable is created
    ...
    class Check(AbstractExpression):
        ...
        def __init__(self, location: Location, expr, validated):
            """
...
def dynvar_bind(
    ...
    with dynvar._bind(value_var.name):
        bind_expr = DynamicVariableBindExpr(
            dynvar, value_var, v, construct(expr), abstract_expr=self
        )
...
def make_node_to_symbol(
    ...
    return CallExpr(
        "Sym", "Get_Symbol", T.Symbol, [node], abstract_expr=abstract_expr
    )
...
def node_to_symbol(
    ...
    return make_node_to_symbol(node_expr, abstract_expr=self)
...
class Try(AbstractExpression):
    ...
    def __init__(self, location: Location, try_expr, else_expr=None):
        """
...
class ArrayLiteral(AbstractExpression):
    ...
    def __init__(self, location: Location, elements=[], element_type=None):
        super().__init__(location)
...
class Literal(AbstractExpression):
    ...
    def __init__(self, location: Location, literal):
        super().__init__(location)
...
class Arithmetic(AbstractExpression):
    ...
    def __init__(self, location: Location, l, r, op):
        """
    ...
    def construct(self):
        ...
        if l.type.is_array_type or l.type.is_string_type:
            ...
            return make_concat(l, r, abstract_expr=self)
